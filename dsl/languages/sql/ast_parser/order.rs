...
impl OrderParser {
    pub fn parse(pair: Pair<Rule>) -> Result<OrderByClause, SqlParseError> {
            ...
            match item.as_rule() {
                Rule::order_item => {
                    ...
                    let column_ref = item_inner.next()
                        .ok_or_else(|| SqlParseError::InvalidInput("Missing column in ORDER BY".to_string()))?;
                        ...
                        match dir.as_rule() {
                            Rule::order_direction => {
                                match dir.as_str().to_uppercase().as_str() {
                                    ...
                                    _ => return Err(SqlParseError::InvalidInput("Invalid sort direction".to_string())),
                                }
                            },
                            _ => return Err(SqlParseError::InvalidInput(format!(
        ...
        if items.is_empty() {
            return Err(SqlParseError::InvalidInput("Empty ORDER BY clause".to_string()));
        }
