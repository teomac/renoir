use crate::dsl::ir::{ComparisonOp, ComplexField, Condition, FilterClause, FilterConditionType, IrPlan, NullCondition, NullOp, BinaryOp};
...
pub(crate) fn process_filter(node: &Value, input_plan: Arc<IrPlan>, conv_object: &ConverterObject) -> Result<Arc<IrPlan>, Box<ConversionError>> {
    ...
    let (filter_clause, _) = process_condition_node(condition_array, 0, conv_object)?;
...
fn process_condition_node(condition_array: &[Value], idx: usize, conv_object: &ConverterObject) -> Result<(FilterClause, usize), Box<ConversionError>> {
    ...
    match node_type {
        "And" | "Or" => process_binary_op_node(condition_array, node_type, idx, conv_object),
        "Not" => process_not_node(condition_array, idx, conv_object),
        "IsNotNull" |  "IsNull"=> process_null_node(condition_array, node_type,  idx, conv_object),
        "EqualTo" | "GreaterThan" |  "LessThan"  | "GreaterThanOrEqual" | "LessThanOrEqual" => process_comparison_node(condition_array, node_type, idx, conv_object),
        _ => Err(Box::new(ConversionError::UnsupportedExpressionType(node_type.to_string()))),
    }
}

/// Process an AND node (left AND right)
fn process_binary_op_node(condition_array: &[Value], op: &str, idx: usize, conv_object: &ConverterObject) -> Result<(FilterClause, usize), Box<ConversionError>> {
    // Process left operand (always the next node)
    let (left_clause, next_idx) = process_condition_node(condition_array, idx + 1, conv_object)?;
    
    // Process right operand (starts after the left branch is complete)
    let (right_clause, final_idx) = process_condition_node(condition_array, next_idx, conv_object)?;
    
    let binary_op = match op {
        "And" => BinaryOp::And,
        "Or" => BinaryOp::Or,
        _ => return Err(Box::new(ConversionError::UnsupportedExpressionType(op.to_string()))),
    };
...
/// Process a NOT node
fn process_not_node(condition_array: &[Value], idx: usize, conv_object: &ConverterObject) -> Result<(FilterClause, usize), Box<ConversionError>> {
    // Get the child index
    let child_idx = idx + 1; // Child is always the next node
    
    // Process the child condition
    let (child_clause, next_idx) = process_condition_node(condition_array, child_idx, conv_object)?;
    ...
    match child_clause {
        FilterClause::Base(FilterConditionType::Comparison(Condition { left_field, operator, right_field })) => {
            ...
            Ok((
                FilterClause::Base(FilterConditionType::Comparison(
                    Condition {
                        left_field,
        ...
        FilterClause::Base(FilterConditionType::NullCheck(NullCondition { field, operator })) => {
            ...
            Ok((
                FilterClause::Base(FilterConditionType::NullCheck(
                    NullCondition {
                        field,
                        operator: negated_operator,
                    }
...
/// Process a NULL node
fn process_null_node(condition_array: &[Value], op: &str, idx: usize, conv_object: &ConverterObject) -> Result<(FilterClause, usize), Box<ConversionError>> {
    ...
    let (field, next_idx) = process_expression(condition_array, attr_idx, conv_object)?;
...
fn process_comparison_node(condition_array: &[Value], node_type: &str,idx: usize, conv_object: &ConverterObject) -> Result<(FilterClause, usize), Box<ConversionError>> {
    let node = &condition_array[idx];
    ...
    let (left_field, next_idx) = process_expression(condition_array, idx + left_idx +1, conv_object)?;
    
    // Process right expression
    let (right_field, final_idx) = process_expression(condition_array, next_idx, conv_object)?;
    ...
    let comparison_op: ComparisonOp = match node_type{
        ...
        _ => return Err(Box::new(ConversionError::UnsupportedExpressionType(node_type.to_string()))),
    };
...
fn process_pow_node(condition_array: &[Value], idx: usize, conv_object: &ConverterObject) -> Result<(ComplexField, usize), Box<ConversionError>> {
    let node = &condition_array[idx];
    
    // Get indices for base and exponent expressions
    let base_idx = node.get("left").and_then(|b| b.as_u64())
        .ok_or_else(|| Box::new(ConversionError::MissingField("left".to_string())))? as usize;
    
    // Process base expression
    let (base_field, next_idx) = process_expression(condition_array, idx + 1 + base_idx, conv_object)?;
    
    // Process exponent expression
    let (exponent_field, final_idx) = process_expression(condition_array, next_idx, conv_object)?;
...
fn process_arithmetic_node(condition_array: &[Value], idx: usize, op: &str, conv_object: &ConverterObject) -> Result<(ComplexField, usize), Box<ConversionError>> {
    let node = &condition_array[idx];
    ...
    // Process left expression
    let (left_field, next_idx) = process_expression(condition_array, idx + 1 + left_idx, conv_object)?;
...
fn process_attribute_reference_node(condition_array: &[Value], idx: usize, conv_object: &ConverterObject) -> Result<(ComplexField, usize), Box<ConversionError>> {
    let node = &condition_array[idx];
...
fn process_literal_node(condition_array: &[Value], idx: usize, conv_object: &ConverterObject) -> Result<(ComplexField, usize), Box<ConversionError>> {
    let node = &condition_array[idx];
...
fn process_expression(condition_array: &[Value], idx: usize, conv_object: &ConverterObject) -> Result<(ComplexField, usize), Box<ConversionError>> {
    if idx >= condition_array.len() {
        return Err(Box::new(ConversionError::InvalidExpression));
    }
    ...
    match expr_type {
        ...
        "Add" => process_arithmetic_node(condition_array, idx, "+", conv_object),
        "Subtract" => process_arithmetic_node(condition_array, idx, "-", conv_object),
        "Multiply" => process_arithmetic_node(condition_array, idx, "*", conv_object),
        "Divide" => process_arithmetic_node(condition_array, idx, "/", conv_object),
        "Pow" => process_pow_node(condition_array, idx, conv_object),
        "Cast" => process_expression(condition_array, idx + 1, conv_object),
