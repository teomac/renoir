...
impl DataFrameASTBuilder {
        ...
        for method in methods {
            match method.as_rule() {
                ...
                Rule::filter_method => {
                    ...
                    base_plan =
                        Self::process_filter_method(method, base_plan, has_join, &table_aliases)?;
                }
                Rule::join_method => {
                    has_join = true;
                    let (updated_plan, new_alias) =
                        Self::process_join_method(method, base_plan, &table_aliases)?;
        ...
        if has_agg {
            if let Some(agg) = agg_method {
                ...
                let group_keys = if let Some(group_method) = groupby_method {
                    ...
                    let groupby_plan = Self::process_groupby_method(
                        group_method,
                        base_plan.clone(),
                        has_join,
                        &table_aliases,
                ...
                base_plan = Self::process_agg_method(
                    agg,
                    group_plan,
                    group_keys,
                    has_join,
            ...
            if let Some(select) = select_method {
                base_plan =
                    Self::process_select_method(select, base_plan, has_join, &table_aliases)?;
            }
        } else {
    ...
    fn process_table_expr(pair: Pair<Rule>) -> Result<(String, Option<String>), Box<IrParseError>> {
        ...
        if let Some(alias_method) = inner.next() {
            if alias_method.as_rule() == Rule::alias_method {
                ...
                let alias_id = alias_method.into_inner().next().ok_or_else(|| {
                    IrParseError::InvalidInput("Missing identifier in alias()".to_string())
                })?;
    ...
    fn process_select_method(
        ...
        for col_item in column_list.into_inner() {
            if col_item.as_rule() == Rule::column_with_alias {
                ...
                let column_ref_pair = parts.next().ok_or_else(|| {
                    IrParseError::InvalidInput("Missing column reference".to_string())
                })?;
                ...
                let column_ref = if column_ref_pair.as_rule() == Rule::column_ref {
                    ...
                    let inner_col = column_ref_pair.into_inner().next().ok_or_else(|| {
                        IrParseError::InvalidInput("Empty column reference".to_string())
                    })?;
                    ...
                    if inner_col.as_rule() == Rule::qualified_column {
                        ...
                        let table = qual_parts
                            ...
                            .ok_or_else(|| {
                                IrParseError::InvalidInput(
                                    "Missing table in qualified column".to_string(),
                                )
                            })?
                            .as_str()
                            .to_string();
                        let column = qual_parts
                            ...
                            .ok_or_else(|| {
                                IrParseError::InvalidInput(
                                    "Missing column in qualified column".to_string(),
                                )
                            })?
                            .as_str()
                            .to_string();
                        ...
                        if has_join
                            && !available_tables.contains(&table)
                            && !table_aliases.contains_key(&table)
                        {
                            return Err(Box::new(IrParseError::InvalidInput(format!(
                        ...
                        ColumnRef {
                            table: if available_tables.len() == 1 {
                                Some(available_tables[0].clone())
                            } else {
                                None
                            },
    ...
    fn parse_filter_condition(
        ...
        if condition.contains("&&") {
            ...
            let left =
                Self::parse_filter_condition(parts[0].trim(), input, has_join, table_aliases)?;
            let right =
                Self::parse_filter_condition(parts[1].trim(), input, has_join, table_aliases)?;
        ...
        let left_field = if parts[0].contains('.') && has_join {
            ...
            if column_parts.len() != 2 {
                return Err(Box::new(IrParseError::InvalidInput(format!(
                    "Invalid qualified column format: {}",
                    parts[0]
                ))));
            }
    ...
    fn process_join_method(
        ...
        let effective_right_alias = right_alias
            .clone()
            .unwrap_or_else(|| right_table_name.clone());
        ...
        let left_col = if left_col_pair.as_rule() == Rule::qualified_column {
            ...
            let table = qual_parts
                .next()
                .ok_or_else(|| {
                    IrParseError::InvalidInput("Missing table in left join column".to_string())
                })?
                .as_str()
                .to_string();
            let column = qual_parts
                .next()
                .ok_or_else(|| {
                    IrParseError::InvalidInput("Missing column in left join column".to_string())
                })?
                .as_str()
                .to_string();
        ...
        let right_col = if right_col_pair.as_rule() == Rule::qualified_column {
            ...
            let table = qual_parts
                .next()
                .ok_or_else(|| {
                    IrParseError::InvalidInput("Missing table in right join column".to_string())
                })?
                .as_str()
                .to_string();
            let column = qual_parts
                .next()
                .ok_or_else(|| {
                    IrParseError::InvalidInput("Missing column in right join column".to_string())
                })?
                .as_str()
                .to_string();
    ...
    fn process_groupby_method(
        ...
        for col_item in column_list.into_inner() {
            if col_item.as_rule() == Rule::column_with_alias {
                let column_ref_pair = col_item.into_inner().next().ok_or_else(|| {
                    IrParseError::InvalidInput("Missing column reference".to_string())
                })?;
    ...
    fn process_agg_method(
        ...
        for agg_expr in agg_list.into_inner() {
            if agg_expr.as_rule() == Rule::agg_expr {
                ...
                let column_ref_pair = parts.next().ok_or_else(|| {
                    IrParseError::InvalidInput("Missing column in aggregation function".to_string())
                })?;
