use crate::dsl::ir::ir_ast_structure::{Group, GroupClause, AggregateType, ComplexField, GroupBaseCondition, NullOp};
use crate::dsl::ir::r_sink::{collect_sink_aggregates, process_grouping_projections};
use crate::dsl::ir::r_utils::check_alias;
use crate::dsl::ir::{BinaryOp, ComparisonOp, IrLiteral, AggregateFunction};
...
impl GroupAccumulatorInfo {
    ...
    fn get_agg_position(&self, agg: &AggregateFunction) -> usize {
        ...
        if let Some((pos, _)) = self.agg_positions.get(&agg_value) {
            *pos
        } else {
...
pub fn process_group_by(group_by: &Group, query_object: &QueryObject) -> String {
    ...
    for col in &group_by.columns {
        if query_object.has_join {
            ...
            let table_name = table_names.first()
    ...
    if let Some(ref group_condition) = group_by.group_condition {
        ...
        group_string.push_str(&create_filter_operation(group_condition, group_by, query_object, &acc_info));
                    ...
                    if left_type != right_type {
                        if !((left_type == "f64" || left_type == "i64") && 
                           (right_type == "f64" || right_type == "i64")) {
                            panic!("Invalid comparison between incompatible types: {} and {}", 
...
fn collect_field_aggregates(
    ...
    match field {
        ComplexField { column_ref: Some(col), .. } => {
            // Validate that the column is either in GROUP BY or used in aggregate
            if !group_by.columns.iter().any(|c| c.column == col.column) {
                panic!("Column {} must appear in GROUP BY or be used in aggregate function", col.column);
            }
        },
        ComplexField { aggregate: Some(agg), .. } => {
        ...
        ComplexField { nested_expr: Some(nested), .. } => {
            ...
            if op == "+" || op == "-" || op == "*" || op == "/" || op == "^" {
                if left_type != "f64" && left_type != "i64" {
                    panic!("Invalid arithmetic operation on non-numeric type: {}", left_type);
                }
                if right_type != "f64" && right_type != "i64" {
                    panic!("Invalid arithmetic operation on non-numeric type: {}", right_type);
                }
            }
        },
        ComplexField { literal: Some(_), .. } => {
        ...
        match value {
            GroupAccumulatorValue::Aggregate(agg_type, _) => {
                let init_value = match agg_type {
                    AggregateType::Sum | AggregateType::Count => {
...
fn create_filter_operation(
    ...
    filter_str.push_str(&process_filter_condition(condition, group_by, query_object, acc_info));
        ...
        GroupClause::Base(base_condition) => {
            match base_condition {
                GroupBaseCondition::Comparison(comp) => {
                    ...
                    let left_expr = process_filter_field(&comp.left_field, group_by, query_object, acc_info);
                    let right_expr = process_filter_field(&comp.right_field, group_by, query_object, acc_info);

                    format!("(({}{}) {} ({}{}))", 
                        left_expr, left_conversion,
                        operator,
                        right_expr, right_conversion
                    )
                },
                GroupBaseCondition::NullCheck(null_check) => {
                    let field_expr = process_filter_field(&null_check.field, group_by, query_object, acc_info);
...
fn process_filter_field(
   ...
   // Added parameter
    ...
    if let Some(ref nested) = field.nested_expr {
        ...
        //get type
        let as_ref = if query_object.get_type(col) == "String" {".as_ref()"} else {""};
        ...
        match lit {
            ...
            IrLiteral::ColumnRef(col_ref) => {
                ...
                if let Some(key_position) = group_by.columns.iter().position(|c| c.column == col_ref.column) {
                    if group_by.columns.len() == 1{
                        format!("x.0")}
                    else{
        ...
        let col = &agg.column;
        let col_access = format!("x.1.{}", agg_pos);
