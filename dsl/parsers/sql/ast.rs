...
impl SqlAST {
            ...
            if let Rule::query = pair.as_rule() {
                ...
                let selection = match select_part.as_rule() {
                    Rule::variable | Rule::table_column => {
                        let col_ref = Self::parse_column_ref(select_part)?;
                    ...
                    Rule::select_expr => {
                        ...
                        SelectType::ComplexValue(col_ref, op, literal)
                    },
                    _ => unreachable!(),
                };
    ...
    fn parse_column_ref(pair: Pair<Rule>) -> Result<ColumnRef, pest::error::Error<Rule>> {
        match pair.as_rule() {
            Rule::table_column => {
                let mut inner = pair.into_inner();
    ...
    fn parse_single_condition(condition_pair: Pair<Rule>) -> Condition {
        let mut inner = condition_pair.into_inner();
        
        let col_ref_pair = inner.next().unwrap();
        let variable = Self::parse_column_ref(col_ref_pair).unwrap();
    ...
    pub fn to_aqua_string(&self) -> String {
        ...
        // FROM clause
        let mut from_str = match &self.from.scan.alias {
            Some(alias) => format!("from {} as {} in input1", self.from.scan.variable, alias),
            None => format!("from {} in input1", self.from.scan.variable),
        };
        
        // Add JOIN if present
        if let Some(join) = &self.from.join {
        ...
        match &self.select.selection {
            SelectType::Simple(col_ref) => {
                parts.push(format!("select {}", col_ref.to_string()));
            },
            SelectType::Aggregate(func, col_ref) => {
                let agg = match func {
                    AggregateFunction::Max => "max",
                };
                parts.push(format!("select {}({})", agg, col_ref.to_string()));
            }
            SelectType::ComplexValue(col_ref, op, val) => {
                ...
                parts.push(format!("select {} {} {}", col_ref.to_string(), op, value));
            }
        }
